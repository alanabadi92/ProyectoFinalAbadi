...
const fetchAfirmaciones = async () => {
    try {
        let resp = await fetch('../json/afirmaciones.json');
...
const fetchDevoluciones = async () => {
    try {
        let resp = await fetch ('../json/devoluciones.json');
...
async function guardarEnJson(objeto,nom) {
    try {
     let resp = await fetch('../json/guardados.json');
...
async function pedirArchivo() {
    const { value: file } = await Swal.fire({
      ...
      if (file) {
        ...
        sessionStorage.setItem("recuperacion",reader.readAsDataURL(file));
